...
public class BezierSplineInspector : Editor {
	...
	private void OnSceneGUI ()
	{
		...
		{
			if(spline.bezierPoints.Length > 0)
			{
				...
				foreach (Point point in spline.bezierPoints)
				{
					...
					controlPointCoordinates = ShowPoint(point.ControlPoint);

					if(point.SecondaryControlPoint != null)
					{
						secondaryControlPointCoordinates = ShowPoint (point.SecondaryControlPoint);
					...
					if(point.previousPoint == spline.GetFirstPoint())
					{
					...
					else if (point.previousPoint != null)
					{
	...
	private void DrawSelectedPointInspector() 
	{
		...
		if (EditorGUI.EndChangeCheck())
		{
			...
			spline.SetControlPointMode(selectedPoint, mode);
			EditorUtility.SetDirty(spline);
		}
	}
	...
	private BezierControlPointMode GetControlPointModeForEditor()
	{
		...
		if (!(selectedPoint is ControlPoint)) 
		{
			mode = (BezierControlPointMode)EditorGUILayout.EnumPopup ("Mode", selectedPoint.BezierControlPointMode);
		}
		return mode;		
	}
	...
	private void ChangeHandlesColorPointMode(Point pointToShow)
	{
		...
		{
			Handles.color = modeColors [(int)(pointToShow as ControlPoint).LinkedPoint.BezierControlPointMode];
		}
		else 
		{
			Handles.color = modeColors [(int)pointToShow.BezierControlPointMode];
		}
	}
}